    ...
    <div class="content-header row">
        ...
        <div class="content-header-right col-md-6 col-12">
            <div class="dropdown float-md-right">
                <a href="{{URL::to('admin/area/create')}}">
                    <button class="btn btn-danger round btn-glow px-2"
                            type="button" aria-expanded="false"> + Add Area
                    </button>
                </a>
                                ...
                                <table class="table table-striped table-bordered file-export">
                                    ...
                                    @foreach($data as $row)
                                        <tr>
                                            ...
                                            <td>{{$row->lon}} </td>
                                            <td>
                                                <a href="{{URL::to('admin/area/'.$row->id.'/status')}}">
                                                    <span
                                                        class="badge {{$row->availability == 'yes' ? 'badge-success' :'badge-danger'}}">{{$row->availability}}</span>
                                                </a>
                                            </td>

                                            <td>
                                                <a href="{{URL::to('admin/area/'.$row->id.'/edit')}}"
                                                   data-toggle="tooltip"
                                                   data-placement="right" title="Edit Area"> <i
                                                        class="ft ft-edit-3"></i></a> &nbsp;
                                                <a href="#"
                                                   data-toggle="tooltip" data-id="{{$row->id}}"
                                                   data-placement="left" title="Delete Area" id="cancel-button"><i
    ...
    <script>
        $('#cancel-button').on('click',function(){
            ...
            swal({
                ...
                .then((isConfirm) => {
                    if (isConfirm) {
                        swal("Deleted!", "Your Area has been deleted.", "success");
                        window.location.href = "area/" + id + "/destroy";
                    } else {
                        swal("Cancelled", "Your Area is safe", "error");
                    }
                });
